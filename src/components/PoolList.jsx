...
export default function PoolList({ onConfirmed, onNotice }) {
  ...
  const { pools, waiting } = useMemo(() => {
    if (!rides.length) return { pools: [], waiting: [] }
    ...
    return {
      pools: groups.filter((g) => g.rides.length >= 2),
      waiting: groups.filter((g) => g.rides.length === 1).flatMap((g) => g.rides),
    }
  }, [rides])
  ...
  return (
    <section className="mt-10">
      ...
      {loading ? (
      ...
      ) : pools.length === 0 && waiting.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-slate-800 bg-slate-900/40 p-10 text-center text-slate-400">
          ...
          <p className="font-semibold text-slate-300">No active rides posted yet</p>
          <p className="mt-1 text-xs text-slate-500">Post your trip above to get matched instantly!</p>
        </div>
      ) : (
        <>
          {/* Active Pools */}
          ...
          {waiting.length > 0 && (
            <div className="mt-8">
              <h3 className="mb-3 text-xs font-bold uppercase tracking-wider text-slate-400 flex items-center gap-2">
                ...
                Waiting for match ({waiting.length})
              </h3>
              <div className="space-y-3">
                {waiting.map((ride) => (
                  <article
                    key={ride.id}
                    className="rounded-xl border border-slate-800 bg-slate-900/60 p-4 shadow-sm backdrop-blur transition hover:border-slate-700"
                  >
                      ...
                      <p className="font-semibold text-slate-200">
                        ...
                        {ride.from && (
                          <span className="ml-1 text-xs font-normal text-slate-400">
                            · Leaving from {ride.from}
                          </span>
                        )}
                      </p>
                      <span className="self-start sm:self-auto rounded-full bg-slate-800 px-2.5 py-0.5 text-xs text-indigo-400 font-medium border border-slate-700/60">
                        {formatDepartureDisplay(ride.departureDate, ride.departureTime, ride.departureTimestamp)}
                      </span>
                    </div>
                    <p className="mt-1.5 text-xs text-indigo-300 flex items-center gap-1.5">
                      <svg className="h-4 w-4 text-indigo-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                      </svg>
                      Headed to {ride.destination}
                    </p>
                    {ride.note && <p className="mt-1 text-xs text-slate-400">{ride.note}</p>}
                  </article>
                ))}
              </div>
            </div>
